...
collect_calls_list([E | Es], Calls) :-
    collect_calls(E, CE), collect_calls_list(Es, CEs), phrase((seq(CE), seq(CEs)), Calls).

collect_calls_bindings([], []).
collect_calls_bindings([bind(_, E) | Bs], Calls) :-
    collect_calls(E, CE), collect_calls_bindings(Bs, CBs), phrase((seq(CE), seq(CBs)), Calls).
...
compile_defs([Def | Rest], Consts, LN0, Slot0, Code) :-
    ...
    phrase((seq(DefCode), seq(RestCode)), Code).
...
compile_def(def(Name, Params, _RetType, _, Body), Consts, LN0, Slot0, Code, LN, SlotAfter) :-
    ...
    phrase(([label(Name)], seq(PrologueCode), seq(BodyCode), [op(ret)]), Code).
...
build_prologue(rest(CountSlot, RestSlot, RestBase, IterSlot), ParamEnv, Consts, LN0, Code, LN) :-
    ...
    phrase((seq(RestPrologueCode), seq(FixedPrologueCode)), Code).

prologue([], []).
prologue([var(_, Addr) | Rest], Code) :-
    prologue(Rest, RestCode),
    phrase(([lit(Addr), op(!)], seq(RestCode)), Code).
...
rest_prologue(CountSlot, RestSlot, RestBase, IterSlot, Consts, LN0, Code, LN) :-
    ...
    phrase((seq(LoopConsts), seq(Consts)), ExtConsts),
    ...
    phrase((seq(PopCount), seq(BindRestPtr), seq(InitIterCode),
            [label(StartL)], seq(CondCode), [zbranch(EndL)],
            seq(AddrCode), [op(!)], seq(DecrCode),
            [branch(StartL), label(EndL)]), Code).
...
compile_body([E | Rest], Env, Consts, LN0, RD, Code, LN) :-
    ...
    phrase((seq(ECode), seq(DropCode), seq(RestCode)), Code).
...
compile_expr(str(Chars), _, _, LN0, _, Code, LN) :-
    ...
    phrase(([branch(EndL), label(DataL)], seq(DataBytes), [byte(0)],
...
compile_expr(binop(Op, A, B), Env, Consts, LN0, RD, Code, LN) :-
    ...
    op_to_vm(Op, VmOps),
    phrase((seq(ACode), seq(BCode), seq(VmOps)), Code).
...
compile_expr(if(Cond, Then, Else), Env, Consts, LN0, RD, Code, LN) :-
    ...
    phrase((seq(CC), [zbranch(ElseL)], seq(TC), [branch(EndL), label(ElseL)],
            seq(EC), [label(EndL)]), Code).
...
compile_expr(let(Bindings, Body), Env, Consts, LN0, RD0, Code, LN) :-
    ...
    phrase((seq(BindCode), seq(BodyCode), seq(CleanupCode)), Code).
...
compile_expr(while(Cond, Body), Env, Consts, LN0, RD, Code, LN) :-
    ...
    phrase(([label(StartL)], seq(CC), [zbranch(EndL)], seq(BC),
            [branch(StartL), label(EndL)]), Code).
...
compile_expr(Expr, Env, Consts, LN0, RD, Code, LN) :-
    ...
    phrase((seq(VC), seq(AC), [op(Op)]), Code).
...
compile_expr(call(Name, Args), Env, Consts, LN0, RD, Code, LN) :-
    ...
    length(Args, TotalArgs),
    RestCount is TotalArgs - FixedArity,
    phrase((seq(ArgsCode), [lit(RestCount), call(Name)]), Code).

%% function call
compile_expr(call(Name, Args), Env, Consts, LN0, RD, Code, LN) :-
    compile_args(Args, Env, Consts, LN0, RD, ArgsCode, LN),
    ( builtin_trap(Name, _, TrapCode) ->
        phrase((seq(ArgsCode), seq(TrapCode)), Code)
    ;
        phrase((seq(ArgsCode), [call(Name)]), Code)
    ).
...
compile_args([A | As], Env, Consts, LN0, RD, Code, LN) :-
    ...
    phrase((seq(AC), seq(RestCode)), Code).
...
compile_let([bind(Name, Expr) | Rest], Env, Consts, LN0, RD0, Code, ExtEnv, LN, RD) :-
    ...
    phrase((seq(ExprCode), [op('>r')], seq(RestCode)), Code).
...
genlabel(N, Prefix, Label, N1) :-
    ...
    phrase((seq(Prefix), seq(NChars)), LabelChars),
